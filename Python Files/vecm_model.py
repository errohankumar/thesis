...
def VECM_forecast(df):
    ...
    vecm_model = VECM(df_diff, k_ar_diff = 26, coint_rank=6).fit()
    vecm_forecast = vecm_model.predict(steps=24)
    vecm_forecast_df = pd.DataFrame(vecm_forecast, columns=df.columns, index= dates)
    vecm_forecast_df = invert_transformation(df, vecm_forecast_df)
    ...
    print(mae)
    '''
    return list(vecm_forecast_df[column_name].values), datetime.now().strftime("%Y-%m-%d")
...
if __name__ == "__main__":
    ...
    c_config = consumer_vecm_local  # default option for consumer
    ...
    while True:
        ...
        if msg.key() != None and msg.key().decode("utf-8") == "Dataset":
            ...
            forecasted, delivery_date = VECM_forecast(pd.read_json(json_df, orient="split"))
            ...
            p.produce(topic, json.dumps(vecm_data), "VECM", callback=delivery_report)
            p.flush()
